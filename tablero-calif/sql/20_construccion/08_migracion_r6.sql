...
drop table if exists proceso.tmp_migracion_r6_clasificado_{IDUNICO} purge;
...
-- --- 5. clasificación --------------------------------------------------------
create table proceso.tmp_migracion_r6_clasificado_{IDUNICO}
stored as parquet
as
select
  p.producto,
...
compute stats proceso.tmp_migracion_r6_clasificado_{IDUNICO};
...
select
  ...
  c.categoria,
  count(*) as clientes
from proceso.tmp_migracion_r6_clasificado_{IDUNICO} c
group by
  c.ingestion_year,
  c.idx_mes_destino,
  c.segmento_anterior,
